...
module.exports = class {
    ...
    constructor(prefix = "/web/", config = {}) {
        this.prefix = prefix;
        this.config = config;
        this.proxifyRequestURL = (url, type) => type ? atob(url.split('_').slice(1).splice(0, 1).join()) + url.split('_').slice(2).join('_') : `_${btoa(url.split('/').splice(0, 3).join('/'))}_/${url.split('/').slice(3).join('/')}`;
    ...
    http(req, res, next = () => res.end('')) {
        ...
        try {
            new URL(this.proxifyRequestURL(req.path, true))
        } catch {
        ...
        if (proxyURL.href.startsWith('https://') || proxyURL.href.startsWith('http://'));
        else return res.end('URL Parse Error');

        delete proxyOptions.headers['host'];

        if (typeof this.config.blacklist == 'object' && this.config.blacklist.length != 0) this.config.blacklist.forEach(blacklisted => proxyURL.hostname == blacklisted ? isBlocked = true : null)
        if (isBlocked) return res.end('The URL you are trying to access is not permitted for use.')

        if (!req.path.startsWith(`/_${btoa(proxyURL.origin)}_/`)) return (res.writeHead(308, {
        ...
        var makeRequest = protocol.request(proxyURL.href, proxyOptions, proxyResponse => {
            ...
            proxyResponse.on('data', data => rawData.push(data)).on('end', () => {
                ...
                proxify.js = buffer => buffer.toString().replace(/(,| |=|\()document.location(,| |=|\)|\.)/gi, str => {
                ...
                else if (proxyResponse.headers['content-type'] && (proxyResponse.headers['content-type'].startsWith('application/javascript') || proxyResponse.headers['content-type'].startsWith('text/javascript') || proxyResponse.headers['content-type'].startsWith('application/x-javascript'))) sendData = proxify.js(sendData.toString());
    ...
    ws(server) {
        new WebSocket.Server({
            ...
            var queryParams = querystring.parse(req.url.split('?').splice(1).join('?')),
                proxyURL, options = {
                    headers: {},
